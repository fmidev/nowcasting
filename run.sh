...
# NECESSARY LAPS data coming from Smartmet Server for Pressure variable often contains missing data for several timesteps. Why???
...
OUTPATH_ELMO=${OUTPATH_ELMO:-'/fmi/$FMI_RUN_ENV/products/cache/nowcasting'}
...
for PARAMETER in Pressure GeopHeight Temperature DewPoint Humidity WindSpeedMS WindDirection HourlyMaximumGust WindUMS WindVMS; do
    ...
    # Copy original input files to elmo cache and remove
    query="cp $DATAPATH$OBSDATA $OUTPATH_ELMO"
    echo $query
    eval $query
    ...
    query="cp $DATAPATH$MODELDATA $OUTPATH_ELMO"
    echo $query
    eval $query
