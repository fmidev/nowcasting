...
def read_nc(image_nc_file,predictability)
    tempds = Dataset(image_nc_file)
    internal_variable = tempds.variables.keys()[-1]
    temps = np.array(tempds.variables[internal_variable][:])



def read_HDF5(image_h5_file,predictability):
...
def main():
    ...
    if options.parameter == 'Precipitation1h':
        first_image_array, quantity, first_timestamp, mask_nodata = read_HDF5("/fmi/data/nowcasting/testdata_radar/opera_rate/T_PAAH21_C_EUOC_20180613120000.hdf",options.predictability)
        second_image_array, quantity_second, second_timestamp, mask_nodata_second = read_HDF5("/fmi/data/nowcasting/testdata_radar/opera_rate/T_PAAH21_C_EUOC_20180613121500.hdf",options.predictability)
    else:
        first_image_array, quantity, first_timestamp, mask_nodata = read_nc("/fmi/data/nowcasting/testdata/obsdata.nc",predictability)
        second_image_array, quantity_second, second_timestamp, mask_nodata_second = read_nc("/fmi/data/nowcasting/testdata/modeldata.nc",predictability)
    ...
    R1 = first_image_array
    R2 = second_image_array
    R_min = quantity_min
    R_max = quantity_max
    logtrans=False
    missingval=nan
    ...
    # #Init file_dicts
    # file_dict_interp=init_filedict_interpolation(options.first_field)
    # file_dict_accum=init_filedict_accumulation(options.first_field)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--first_field',
                        help='Two-dimensional array containing the first field.')
    parser.add_argument('--second_field',
                        help='Two-dimensional array containing the second  field.')
